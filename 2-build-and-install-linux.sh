#!/usr/bin/env bash
# ===========================================================================
#  Mriya - build the screensaver for Linux and install it into
#  XScreenSaver
#
...
if [ -n "$PREFIX" ]; then LIST_CMD="$HACK -root"; else LIST_CMD="mriya -root"; fi
...
register_user() {
    if [ ! -f "$XSS_FILE" ]; then
        ...
        say "       to add Mriya to the list."
        return 0
    fi
    if grep -q 'mriya' "$XSS_FILE"; then
        say "Already in $XSS_FILE"
        return 0
    fi
    cp "$XSS_FILE" "$XSS_FILE.bak-mriya"
    ...
    ' "$XSS_FILE.bak-mriya" > "$XSS_FILE"
    if grep -q 'mriya' "$XSS_FILE"; then
        say "Added to $XSS_FILE (backup: $XSS_FILE.bak-mriya)"
    else
        cp "$XSS_FILE.bak-mriya" "$XSS_FILE"
...
unregister_user() {
    [ -f "$XSS_FILE" ] && grep -q 'mriya' "$XSS_FILE" || return 0
    cp "$XSS_FILE" "$XSS_FILE.bak-mriya"
    grep -v 'mriya' "$XSS_FILE.bak-mriya" > "$XSS_FILE"
    say "Removed from $XSS_FILE (backup: $XSS_FILE.bak-mriya)"
}
...
if [ "$UNINSTALL" = 1 ]; then
    echo
    say "Mriya - uninstall"
    for f in "$HACK" "$CONFDIR/mriya.xml"; do
        if [ -e "$f" ]; then
    ...
    unregister_user
    say "Saved settings are kept in ${XDG_CONFIG_HOME:-$HOME/.config}/mriya"
    echo
    exit 0
fi
...
if [ "$SYSTEM_SDL" = 1 ]; then
    SDL_ARGS+=(-DMR_SYSTEM_SDL=ON)
elif [ -f "$WORKENV/SDL3-$V_SDL/CMakeLists.txt" ]; then
    SDL_ARGS+=(-DMR_SYSTEM_SDL=OFF "-DFETCHCONTENT_SOURCE_DIR_SDL3=$WORKENV/SDL3-$V_SDL")
else
...
echo
say "Done. Open xscreensaver-settings and pick \"Mriya\"."

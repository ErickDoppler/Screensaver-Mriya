...
vec3 milky_way(vec3 d) {
    // a band along a tilted great circle
    vec3 n = normalize(vec3(0.31, 0.82, -0.48));
    float b = dot(d, n);
    vec3 t = normalize(cross(n, vec3(0.0, 0.0, 1.0)));
    float lon = atan(dot(d, cross(n, t)), dot(d, t));
    float band = exp(-b * b * 22.0);
    float neb = snoise2(vec2(lon * 9.0, b * 30.0)) * 0.6 + snoise2(vec2(lon * 23.0, b * 70.0)) * 0.4;
    float dust = smoothstep(0.35, 0.7, snoise2(vec2(lon * 14.0 + 3.0, b * 50.0)));
    return vec3(0.75, 0.8, 1.0) * band * (0.4 + 0.8 * neb) * (1.0 - 0.6 * dust * exp(-b * b * 200.0));
}
...
void main() {
    ...
    if (!ground) {
        ...
        if (uStars > 0.0) {
            vec3 st = stars(d) + milky_way(d) * 0.05;
            col += st * uStars * T;
        }
        col += aurora(d) * T;
    }
